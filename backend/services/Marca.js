...
const index = async data => {
    try {
        const result = await Marca.findAndCountAll({
            where: getWhere(data),
...
const store = async data => {
    try {
        const result = await Marca.create(data);
...
const update = async (data, id) => {
    try {
        await Marca.update(data, { where: { id } });
        const result = Marca.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await Marca.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
