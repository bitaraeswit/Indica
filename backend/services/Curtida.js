const { Curtida, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await Curtida.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await Curtida.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await Curtida.create(data);
...
const update = async (data, id) => {
    try {
        await Curtida.update(data, { where: { id } });
        const result = Curtida.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await Curtida.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
