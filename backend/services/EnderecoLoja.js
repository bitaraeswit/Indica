const { EnderecoLoja, Loja, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await EnderecoLoja.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await EnderecoLoja.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await EnderecoLoja.create(data);
...
const update = async (data, id) => {
    try {
        await EnderecoLoja.update(data, { where: { id } });
        const result = EnderecoLoja.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await EnderecoLoja.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
