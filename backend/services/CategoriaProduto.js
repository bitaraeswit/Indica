const { CategoriaProduto, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await CategoriaProduto.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await CategoriaProduto.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await CategoriaProduto.create(data);
...
const update = async (data, id) => {
    try {
        ...
        const result = CategoriaProduto.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await CategoriaProduto.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
