...
const store = async data => {
    try {
        const result = await Notificacao.create(data);
...
const update = async (data, id) => {
    try {
        await Notificacao.update(data, { where: { id } });
        const result = Notificacao.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await Notificacao.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
