const { Cidade, Estado, Localidade, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await Localidade.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await Localidade.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await Localidade.create(data);
...
const update = async (data, id) => {
    try {
        await Localidade.update(data, { where: { id } });
        const result = Localidade.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await Localidade.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
