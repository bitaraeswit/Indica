const { ImagemProduto, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await ImagemProduto.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await ImagemProduto.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await ImagemProduto.create(data);
...
const update = async (data, id) => {
    try {
        await ImagemProduto.update(data, { where: { id } });
        const result = ImagemProduto.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await ImagemProduto.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
