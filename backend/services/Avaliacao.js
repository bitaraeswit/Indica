const { Avaliacao, Usuario, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await Avaliacao.findAndCountAll({
            where: getWhere(data),
...
const store = async data => {
    try {
        const result = await Avaliacao.create(data);
...
const update = async (data, id) => {
    try {
        ...
        const result = Avaliacao.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await Avaliacao.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
