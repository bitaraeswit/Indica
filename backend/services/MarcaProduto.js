const { MarcaProduto, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await MarcaProduto.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await MarcaProduto.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await MarcaProduto.create(data);
...
const update = async (data, id) => {
    try {
        await MarcaProduto.update(data, { where: { id } });
        const result = MarcaProduto.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await MarcaProduto.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
