const { MetodoPagamento, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await MetodoPagamento.findAndCountAll({
            where: getWhere(data),
...
const store = async data => {
    try {
        const result = await MetodoPagamento.create(data);
...
const update = async (data, id) => {
    try {
        await MetodoPagamento.update(data, { where: { id } });
        const result = MetodoPagamento.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await MetodoPagamento.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
