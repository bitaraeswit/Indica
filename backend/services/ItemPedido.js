const { ItemPedido, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await ItemPedido.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await ItemPedido.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await ItemPedido.create(data);
...
const update = async (data, id) => {
    try {
        await ItemPedido.update(data, { where: { id } });
        const result = ItemPedido.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await ItemPedido.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
