const { TamanhoProduto, Sequelize } = require('@models');
...
const index = async data => {
    try {
        const result = await TamanhoProduto.findAndCountAll({
            where: getWhere(data),
...
const show = async data => {
    try {
        const result = await TamanhoProduto.findOne({
            where: getWhere(data),
            include: getInclude(data),
            order: [],
        });
...
const store = async data => {
    try {
        const result = await TamanhoProduto.create(data);
...
const update = async (data, id) => {
    try {
        await TamanhoProduto.update(data, { where: { id } });
        const result = TamanhoProduto.findByPk(id);
        return Object.assign(data, result.get({ plain: true }));
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
...
const destroy = async data => {
    try {
        ...
        return await TamanhoProduto.destroy({ where: data });
    } catch (error) {
        throw new Error(error.errors.map(e => e.message));
    }
};
